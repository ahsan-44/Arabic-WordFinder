...
public class TestIAP : MonoBehaviour
{
    ...
    void OnGUI()
    {
        ...
        if (showButtons)
        {
            if (!IAPManagerGely.Instance.IsInitialized())
            {
                if (initializationInProgress == false)
                {
                    if (Button("Initialize"))
                    {
                        ...
                        //Initialize IAP
                        IAPManagerGely.Instance.InitializeIAPManager(InitializeResult);
                    }
                }
            }
            else
            {
                if (purchaseInProgress == false)
                {
                    ...
                    if (consumableProducts.Count > 0)
                    {
                        ...
                        if (Button("BUY " + consumableProducts[indexNumberConsumable].name + " " + IAPManagerGely.Instance.GetValue(consumableProducts[indexNumberConsumable].name) + " Coins " + IAPManagerGely.Instance.GetLocalizedPriceString(consumableProducts[indexNumberConsumable].name)))
                        {
                            purchaseInProgress = true;

                            //this function can be called like this
                            //IAPManager.Instance.BuyProduct(ShopProductNames.Consumable1, ProductBought);
                            IAPManagerGely.Instance.BuyProduct(consumableProducts[indexNumberConsumable].name, ProductBought);
                        }
                    }
                    else
                    {
                    ...
                    if (nonCOnsumableProducts.Count > 0)
                    {
                        ...
                        if (Button("BUY " + nonCOnsumableProducts[indexNumberNonConsumable].name + " " + IAPManagerGely.Instance.GetLocalizedPriceString(nonCOnsumableProducts[indexNumberNonConsumable].name)))
                        {
                            purchaseInProgress = true;

                            //this function can be called like this
                            //IAPManager.Instance.BuyProduct(ShopProductNames.NonConsumable1, ProductBought);
                            IAPManagerGely.Instance.BuyProduct(nonCOnsumableProducts[indexNumberNonConsumable].name, ProductBought);
                        }
                    }
                    else
                    {
                    ...
                    if (subscriptions.Count > 0)
                    {
                        ...
                        if (Button("BUY " + subscriptions[indexNumberSubscription].name + " " + IAPManagerGely.Instance.GetLocalizedPriceString(subscriptions[indexNumberSubscription].name)))
                        {
                            purchaseInProgress = true;

                            //this function can be called like this
                            //IAPManager.Instance.BuyProduct(ShopProductNames.Subscription1, ProductBought);
                            IAPManagerGely.Instance.BuyProduct(subscriptions[indexNumberSubscription].name, ProductBought);
                        }
                    }
                    else
                    {
                    ...
                    {
                        IAPManagerGely.Instance.RestorePurchases(ProductBought, RestoreDone);
                    }
    ...
    {
        if (IAPManagerGely.Instance.debug)
        {
    ...
    private void ProductBought(IAPOperationStatus status, string message, StoreProduct product)
    {
        ...
        if (status == IAPOperationStatus.Success)
        {
            ...
            if (product.productType == ProductTypeG.Consumable)
            {
                coins += product.value;
            }
            ...
            if (product.productType == ProductTypeG.NonConsumable)
            {
                if (product.active)
                {
            ...
            if (product.productType == ProductTypeG.Subscription)
            {
                if (product.active)
                {
    ...
    private void InitializeResult(IAPOperationStatus status, string message, List<StoreProduct> shopProducts)
    {
        ...
        if (status == IAPOperationStatus.Success)
        {
            ...
            for (int i = 0; i < shopProducts.Count; i++)
            {
                ...
                {
                    case ProductTypeG.Consumable:
                        consumableProducts.Add(new MyStoreProducts(IAPManagerGely.Instance.ConvertNameToShopProduct(shopProducts[i].productName), shopProducts[i].active));
                        break;
                    case ProductTypeG.NonConsumable:
                        nonCOnsumableProducts.Add(new MyStoreProducts(IAPManagerGely.Instance.ConvertNameToShopProduct(shopProducts[i].productName), shopProducts[i].active));
                        break;
                    case ProductTypeG.Subscription:
                        subscriptions.Add(new MyStoreProducts(IAPManagerGely.Instance.ConvertNameToShopProduct(shopProducts[i].productName), shopProducts[i].active));
                        break;
                }
            }
    ...
    void DropDown(int maxButtons, ref int indexNumber, ref bool showDropDown, List<MyStoreProducts> coll)
    {
        ...
        for (int index = 0; index < coll.Count; index++)
        {
            ...
            if (GUI.Button(new Rect(padding, (index * buttonHeight), buttonWidth - 2 * padding, buttonHeight), coll[index].name + " " + IAPManagerGely.Instance.GetLocalizedPriceString(coll[index].name)))
            {
                showDropDown = false;
                indexNumber = index;
            }
